...
describe("convertToEpub", () => {
   ...
   */
  it("round-trips a real Kindle file back to a readable EPUB", async () => {
  ...
  it("reports a corrupt file as failed, without throwing", async () => {
    await withTmpDir(async (dir) => {
  ...
  it("reports a missing source file as failed rather than crashing the upload", async () => {
    await withTmpDir(async (dir) => {
  ...
  it("never reports success without a file behind it", async () => {
