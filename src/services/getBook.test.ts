...
describe("updateBookRecord", () => {
  ...
  it("creates without an inline cover upload and patches the cover afterwards", async () => {
    ...
    try {
      ...
      expect(followUp).resolves.toBe("completed");
  ...
  it("skips the follow-up when the record is already complete", async () => {
    ...
    expect(followUp).resolves.toBe("nothing");
    expect(names()).toEqual(["putRecord"]);
  });
  ...
  it("gives up as a conflict when the CAS race never resolves", async () => {
    ...
    try {
      ...
      expect(followUp).resolves.toBe("conflict");
  ...
  it("keeps the start date when progress is saved on a book already Reading", async () => {
    ...
    // An explicit Reading choice preserves the existing start date.
    const { book } = await updateBookRecord({
      ctx,
      agent,
      hiveId: HIVE_ID,
  ...
  it("persists completion and a finish date when saving the final page alone", async () => {
    const original = baseRecord({ status: READING, startedAt: "2026-03-01T09:00:00.000Z" });
    const { agent } = fakePds({ value: original, cid: "cid0" });
    await seedRow(original, "cid0", { status: READING, startedAt: original.startedAt });
    const { book } = await updateBookRecord({
      ctx,
      agent,
      hiveId: HIVE_ID,
      updates: {
        bookProgress: { currentPage: 658, totalPages: 658, updatedAt: "2026-08-20T00:00:00.000Z" },
      },
    });
    expect(book.status).toBe(FINISHED);
    expect(book.finishedAt).toBeTruthy();
    expect(book.startedAt).toBe(original.startedAt);
    expect(book.bookProgress?.percent).toBe(100);
    const reloaded = await getUserBook({ ctx, agent, hiveId: HIVE_ID });
    expect(reloaded?.status).toBe(FINISHED);
    expect(reloaded?.finishedAt).toBe(book.finishedAt);
  });
