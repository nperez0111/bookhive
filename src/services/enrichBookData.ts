...
import { normalizeGoodreadsId, upsertBookIdentifiers } from "../data/bookIdentifiers";
import { Semaphore, withTimeout } from "../lib/semaphore";
import { errorMessage } from "../lib/errors";
import { deferVerdict, verdictFields } from "../core/enrichVerdict";

// Bounds enrichment for every caller — the backstop against reintroducing the
// unbounded fan-out that once caused a production OOM.
...
export async function enrichBookWithDetailedData(
  ...
  try {
    ...
    // Slot acquisition is outside the deadline — waiting for a free scraper
    // is not the book's fault.
    const release = await enrichmentSemaphore.acquireSlot();
    const work = enrich(book, ctx);
    // Hold the slot until the work actually settles: withTimeout can't cancel
    // an in-flight scrape, so releasing on the deadline would let more than
    // `limit` scrapes run at once.
    ...
    ctx.addWideEventContext({
      ...
      // Our own slowness or a thrown exception is not Goodreads' verdict on this book — never spend an attempt on it.
      ...verdictFields(deferVerdict("enrich_exception")),
    });
...
async function enrich(
  ...
  // Only use valid (numeric) Goodreads IDs; reject Amazon/Kindle identifiers like kca://book/amzn1.
  ...
  await ctx.db
    ...
    .set({
      ...
      description: detailedData.book.description || book.description,
      rating: Math.round(detailedData.work.averageRating * 1000), // hive_book.rating is stored ×1000
  ...
  // Prefer the numeric sourceId from the search API over the kca:// ID from the page scrape.
