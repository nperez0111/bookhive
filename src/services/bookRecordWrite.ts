/**
 * CAS read/write of a book record — the write must fail if the PDS holds
 * something we have not seen, since the merge is computed from local state.
 */
import type { SessionClient } from "../auth/client";
import { ids, Book as BookRecord } from "../bsky/lexicon/index";
...
export async function getBookRecord({
  ...
  // A record failing our validator is still the user's record — merge onto it
  // rather than refuse, or the offending field can never be overwritten.
