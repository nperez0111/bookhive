 ...
 * neither is needed to render, but both are slow (a cover fetch, a scrape),
 * so nobody clicking a status should wait for them.
 * CAS'd on the cid the request wrote; on conflict it re-reads the winner and
 * re-applies on top, since losing that race to the user's own next edit is
 * the common case and no later write re-supplies the cover.
 */
  ...
  const run = async (): Promise<FollowUpOutcome> => {
    ...
    for (let attempt = 0; attempt < MAX_CAS_ATTEMPTS; attempt++) {
      ...
      if (written.ok) {
        ...
        // patched — replaying the whole row would revert column-only writes
        // (owned, KOSync progress) made during the window, which the CAS can't protect since they aren't in the record.
        await ctx.db
