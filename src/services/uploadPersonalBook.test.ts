...
import { koreaderPartialMD5, MIN_COVER_DIMENSION } from "../core/bookMetadata/index";
import { makeCbz, makeEpub, makeFb2, makeSvgCover, PNG_1 } from "../core/bookMetadata/testFixtures";
...
describe("uploadPersonalBook — rejections", () => {
  ...
  it("caps a chunked body with no declared length", async () => {
    ...
    //
    // Run against a 1 MB `maxBytes` rather than the real 100 MB. The mechanism
    // under test is "stop reading once `written` passes `cap`", which is cap-
    // independent — and `pull`, not `start`, so the producer is demand-driven:
    // if the writer ever stopped enforcing the cap this hangs or OOMs instead
    // of quietly passing, and the previous version's `start` enqueued the whole
    // 100 MB into the stream's queue up front, which is what made this the
    ...
    const oversized = new ReadableStream<Uint8Array>({
      pull(controller) {
        ...
        // Only ever needs to exceed the cap; a bound keeps a broken writer from
        // spinning forever.
        if (produced > CAP * 4) controller.close();
      },
    });
    ...
    expect(await tmpEntries()).toEqual([]);
    // The writer must stop pulling, not drain the producer dry.
    expect(produced).toBeLessThanOrEqual(CAP * 4);
  });
  ...
  it("caps an in-memory byte source too", async () => {
    ...
    // Against a small `maxBytes`, for the same reason as the streaming case
    // above: allocating a real 100 MB buffer tests the allocator, not the cap.
    const CAP = 1024 * 1024;
    ...
    const result = await uploadPersonalBook({
      ...
      userDid: DID,
      filename: "huge.epub",
      source: { kind: "bytes", bytes: new Uint8Array(CAP + 1) },
      maxBytes: CAP,
    });
  ...
  it("the real ceiling is 100 MB", () => {
    // The tests above deliberately run against an injected cap, so pin the
    // production value separately — otherwise nothing asserts it at all.
    expect(MAX_PERSONAL_BOOK_BYTES).toBe(100 * 1024 * 1024);
  });
});
