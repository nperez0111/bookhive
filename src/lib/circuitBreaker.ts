 ...
 *
 * Deliberately not used for scraping any more. A breaker refuses work before
 * sending it, which is only right when the refused thing has no other way to
 * fail safely — `enrich_queue` can defer a book without charging it an
 * attempt, so `scrapers/waf/solver.ts` bounds cost with single-flight and a
 * minimum interval instead, and every book still gets its request. See that
 * file for what putting a breaker in front of it cost.
 */
