...
describe("Semaphore", () => {
  ...
  it("releases the slot when the task throws", async () => {
    ...
    expect(sem.run(async () => Promise.reject(new Error("boom")))).rejects.toThrow("boom");
    expect(sem.active).toBe(0);
    expect(sem.run(async () => "ok")).resolves.toBe("ok");
  });
  ...
  it("sheds load past maxPending instead of queueing forever", async () => {
    ...
    expect(sem.run(async () => "rejected")).rejects.toBeInstanceOf(SemaphoreFullError);

    gate.resolve();
    await running;
    expect(queued).resolves.toBe("queued");
  });
  ...
  it("times out a caller that waits too long for a slot", async () => {
    ...
    expect(sem.run(async () => "never")).rejects.toBeInstanceOf(SemaphoreTimeoutError);
    expect(sem.pending).toBe(0);
  ...
  it("rejects waiters on clearPending", async () => {
    ...
    expect(queued).rejects.toThrow("shutting down");
...
describe("withTimeout", () => {
  it("passes through a fast result", async () => {
    expect(withTimeout(Promise.resolve("ok"), 1000, "task")).resolves.toBe("ok");
  });

  it("rejects with the label when the deadline passes", async () => {
    expect(withTimeout(Bun.sleep(50), 5, "slow task")).rejects.toThrow(
