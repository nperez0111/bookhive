...
function quotedString(value: string): string {
  // Matching control characters is the point: a bare CR or LF in a header value
  // is response splitting, and they cannot be escaped, only dropped. The class
  // is already spelled with Unicode escapes, which is what the rule's own advice
  // asks for, so the rule fires regardless.
