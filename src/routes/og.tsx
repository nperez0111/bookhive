...
/**
 * OG image generation routes using Takumi.
 * Mount at /og.
 */
...
// Light warm amber palette matching the app
const COLORS = {
...
const makeOgResponse = (jsx: React.ReactElement) =>
  new ImageResponse(jsx, OG_RESPONSE_OPTIONS);
...
function StarRating({ rating, max = 5, size = 28 }: { rating: number; max?: number; size?: number }) {
...
function BookOgCard({
  ...
  return (
    <div
      ...
      style={{ backgroundColor: COLORS.bg, fontFamily: "Geist" }}
    >
      {coverUrl && (
        <img
          src={coverUrl}
      ...
      >
        <div tw="flex flex-col" style={{ gap: 14 }}>
          ...
          {shortDesc && (
            <div tw="text-lg leading-relaxed" style={{ color: COLORS.textMuted }}>
              {shortDesc}
            </div>
          )}
        </div>
...
/** OG card for a user's yearly reading stats */
function StatsOgCard({
  handle,
  displayName,
  avatarUrl,
  year,
  ...
  return (
    <div tw="flex w-full h-full" style={{ backgroundColor: COLORS.bg, fontFamily: "Geist" }}>
      ...
      >
        ...
        <div tw="font-bold text-center" style={{ color: COLORS.text, fontSize: 22, lineHeight: 1.2 }}>
          {name}
        </div>
        ...
        <div tw="flex" style={{ gap: 32 }}>
          {averageRating != null && (
            <div tw="flex flex-col" style={{ gap: 2 }}>
              ...
              <span tw="text-xl" style={{ color: COLORS.textMuted }}>avg rating</span>
            </div>
          )}
          {pagesRead > 0 && (
            <div tw="flex flex-col" style={{ gap: 2 }}>
              ...
              <span tw="text-xl" style={{ color: COLORS.textMuted }}>pages read</span>
            </div>
          )}
          {topGenres.length > 0 && (
            <div tw="flex flex-col" style={{ gap: 2 }}>
              ...
              <span tw="text-xl" style={{ color: COLORS.textMuted }}>top genre</span>
            </div>
          )}
        </div>
        ...
        {topGenres.length > 1 && (
          <div tw="flex flex-wrap" style={{ gap: 8 }}>
            {topGenres.slice(1).map((g) => (
              <div
                key={g.genre}
                tw="text-base px-3 py-1 rounded-full"
                style={{
...
/** OG card for a user's profile page */
  ...
  return (
    <SidebarLayout sidebarWidth={300} sidebarPadding="36px 24px" covers={recentCovers}>
      <div tw="flex flex-col items-center" style={{ gap: 10 }}>
        ...
        <div tw="font-bold text-center" style={{ color: COLORS.text, fontSize: 26, lineHeight: 1.2 }}>
          {name}
        </div>
        ...
        <div tw="flex flex-col items-center" style={{ gap: 2 }}>
          <div tw="font-bold text-center" style={{ color: COLORS.primary, fontSize: 48, lineHeight: 1 }}>
            {totalBooks}
          </div>
          <div tw="text-base text-center" style={{ color: COLORS.textMuted }}>
            books on BookHive
...
/** OG card for author and genre pages — identical layout, just different label */
function LabeledCoverCard({
  label,
  name,
  totalBooks,
  covers,
  ...
  return (
    <SidebarLayout sidebarWidth={320} sidebarPadding="44px 36px" covers={covers}>
      <div tw="flex flex-col" style={{ gap: 14 }}>
        ...
        <div tw="font-bold leading-tight" style={{ color: COLORS.text, fontSize: 72, lineHeight: 1.0 }}>
          {shortName}
        </div>
...
const app = new Hono<AppEnv>()
  ...
  .get("/profile/:handle/stats/:year", async (c) => {
    ...
    const avatarUrl = profile?.avatar
      ? `${getOrigin(c)}/images/w_176/${profile.avatar}`
      : undefined;
    ...
    return makeOgResponse(
      <StatsOgCard
        ...
        topGenres={stats.topGenres.slice(0, 3)}
  ...
  .get("/profile/:handle", async (c) => {
    ...
    const origin = getOrigin(c);
    const [profile, totalRow, recentBooks, genreRows] = await Promise.all([
  ...
  .get("/author/:author", async (c) => {
    ...
    const [totalRow, books] = await Promise.all([
      c.get("ctx").db
