...
import { createTestDb as makeDb } from "../test/db";
...
/** `getProfiles` short-circuits to the public client with no session, so avatars come back empty — none of these assertions check avatars. */
...
describe("collapseBursts", () => {
  ...
  it("groups by actor across mixed verbs, and labels the mix 'logged'", () => {
    // Actor-keyed, not (actor, verb)-keyed: a re-sync interleaves statuses row by row, so a verb-sensitive key would break the run apart.
    const items = [
  ...
  it("emits a LONG open trailing run as a truncated burst rather than deferring it", () => {
    // A single actor dominating the fetch (mid-import) must still fill the page, not defer to one or two rows.
...
describe("getActivityFeed", () => {
  ...
  it("orders by indexedAt, not createdAt — the reported bug", async () => {
    // createdAt order is deliberately the reverse of indexedAt order, to catch sorting by the wrong column.
  ...
  it("terminates and yields each row once when every timestamp is identical", async () => {
    // Without `uri` as a tiebreaker, SQLite may order ties differently between requests and silently drop rows from a paginated feed.
    const ts = iso(500);
    for (let i = 0; i < 12; i++) {
      await insertBook({
        userDid: `did:plc:u${i}`,
  ...
  it("advances the cursor past every row of a collapsed burst", async () => {
    // The cursor must come from the last raw row consumed, not the last display row — a cursor taken from a collapsed burst's newest row would re-serve the same burst forever.
    for (let i = 0; i < 10; i++) {
      await insertBook({
        userDid: ALICE,
        hiveId: `bk_a${i}`,
...
describe("feed query plans", () => {
  // bun:sqlite is synchronous across all worker processes, so assert the plan rather than trusting the planner — this database is never ANALYZEd.
  beforeEach(async () => {
    seq = 0;
    ({ db, sqlite } = await makeDb());
  });
  ...
  // The IN-driven tabs cannot get ordering from an index, so a temp B-tree over the matched rows is inherent — what must never happen is a full scan of user_book feeding that sort.
