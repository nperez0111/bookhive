 ...
 *
 * The query-plan tests below guard the covering index, which is only used
 * because the queries name it with `INDEXED BY` — this database has no
 * `sqlite_stat1`, so without stats the planner would otherwise prefer the
 * UNIQUE `sqlite_autoindex_hive_book_1` and fetch the whole row.
 */
...
describe("author aggregate query plans", () => {
  // Guards the production regression: without INDEXED BY this falls back to sqlite_autoindex_hive_book_1, fetching the whole row for every book.
  const AGGREGATE = `
...
describe("getFeaturedAuthors", () => {
  ...
  it("is a strict prefix of getAuthorStats", async () => {
    // /explore/authors used to run the same GROUP BY twice per render, once at LIMIT 8 and once at LIMIT 500.
    const kv = newKv();
