...
describe("streamPersonalBook", () => {
  ...
  // These routes are excluded from hono's etag() middleware (which buffers the whole body), so this branch must produce the 304 itself — otherwise an e-reader re-downloads every book on every sync.
...
describe("Content-Disposition", () => {
  ...
  // encodeURIComponent leaves `'` alone, and `'` is the delimiter inside an RFC 8187 ext-value — that truncated every title with an apostrophe.
...
describe("serving a derived EPUB", () => {
  ...
  // contentHash is the hash of the original; reusing it bare would tell a client holding the MOBI it already has the EPUB, keeping the stale copy forever.
  ...
  // epubPath points at a second file nothing re-derives — if it goes missing, failing the download would lose the user the original too, which is still on disk.
  ...
  // filename* used to be derived from the canonical ASCII name, so a title with no ASCII form lost itself entirely on conversion.
