...
import { retryVerdict, verdictFields } from "../core/enrichVerdict";
...
describe("enrichQueue", () => {
  ...
  it("reschedules with backoff when the scrape fails", async () => {
    ...
    await drainEnrichmentQueue(db, logger, {
      // An explicit `retry` verdict — this test is about what a real answer costs, not the default disposition.
      enrich: fakeEnrich({ enrichment: "failed", ...verdictFields(retryVerdict("bad_payload")) }),
    });
    ...
    expect(row!.lastError).toBe("bad_payload");
...
describe("convergence", () => {
  /** A row that hit MAX_ATTEMPTS used to be deleted without recording anything on hive_book, so the next page view re-enqueued the same book — a perpetual-motion machine. */
  const alwaysFails = (async (
    ...
    ctx.addWideEventContext({
      enrichment: "failed",
      ...verdictFields(retryVerdict("bad_payload")),
    });
  }) as unknown as EnrichFn;
  ...
  it("reports exhausted books in the heartbeat, not a structurally-zero count", async () => {
    // The gauge used to count enrich_queue rows at MAX_ATTEMPTS, but those rows are deleted in the same call that stamps enrichFailedAt, so it always read 0 — it must be read off hive_book.
    const { db } = await createTestDb();
    const { logger, events } = makeLogger();
    await insertBook(db, HIVE_ID);
    await enqueueEnrichment(db, HIVE_ID);
...
/** `attempts` counts answers from Goodreads, so only an answer may spend one — a refusal the app generated itself (e.g. `circuit_open`) must not. */
describe("dispositions", () => {
  ...
  it("gives up on a deferred book once it has been queued a week", async () => {
    // Defers are free, so this age ceiling is the only thing that bounds a queue whose blocker never clears.
    const { db } = await createTestDb();
    const { logger, events } = makeLogger();
    await insertBook(db, HIVE_ID);
    await enqueueEnrichment(db, HIVE_ID);
