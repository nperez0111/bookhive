 ...
 *     on the public pages.
 *
 * This lives in one place because three layers each apply it to a different
 * view of the response: `cacheControl()` (src/routes/lib.ts) before the final
 * status is known, `anonPageCache` (src/middleware/anon-page-cache.ts), and the
 * nitro `response` hook (server/plugins/cache-headers.ts), the only layer that
 * sees the response actually leaving the process.
 */

/** iron-session cookie (see getSessionConfig in src/auth/session.ts). */
