...
export function parseEpub(bytes: Uint8Array, fallbackTitle: string): BookMetadata {
  ...
  // size of every image without inflating any of them — returning `false` from
  // the filter still walks the central directory for free. See pass 2 below
  // for the one entry we do inflate.
...
function inflateCover(bytes: Uint8Array, entry: ImageEntry | undefined): BookCover | undefined {
  ...
  // `originalSize` is the archive's own claim; check the actual bytes too, since
