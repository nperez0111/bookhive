/**
 * Filename-derived identity for e-reader documents, used (in order of
 * confidence) to match a `sync_document.documentHash`, another file's
 * normalized name, or a `hive_book` title/author.
 *
 * **`filenameKey` and `koreaderFilenameHash` are persisted** (`personal_book`
 * and `sync_document` columns), so they are a stored index of a pure function:
 * changing their output silently splits old rows from new ones and needs a
 * follow-up migration to recompute every row, not just a fix here.
 */
 ...
 * a dot-number ("Foundation Vol.2", "Hitchhiker's 1.5"). Stripping the
 * extension is also what lets the key survive a calibre conversion round trip
 * (.epub on the desktop, .azw3 on the Kindle).
 */
