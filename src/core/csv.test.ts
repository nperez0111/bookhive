...
`;
      ...
      expect(books[0]).toMatchObject({
        ...
        sponsoredReview: false,
        // Hardcover's "Stopped" is a did-not-finish. It used to import as
        // want-to-read because the status enum `csv.ts` imported had no
        // `abandoned` member — the drifted duplicate that lived in types.ts.
        status: "buzz.bookhive.defs#abandoned",
        tags: "",
