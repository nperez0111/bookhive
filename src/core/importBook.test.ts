...
describe("importBook utilities", () => {
  ...
  describe("normalizeStorygraphRating", () => {
    it("doubles onto the 1-10 stars scale", () => {
    ...
    // StoryGraph exports quarter stars, which have no exact 1-10 representation.
    // Rounding keeps them within half a display star; the previous `parseInt`
    // truncation always cost a full half star.
    it("rounds a quarter star to the nearest half", () => {
      expect(normalizeStorygraphRating(4.25)).toBe(9);
      expect(normalizeStorygraphRating(4.75)).toBe(10);
    });
