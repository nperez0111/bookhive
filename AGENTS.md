...
- `/` → `src/pages/marketing.tsx` — landing for signed-out visitors; **302s to `/home` when the `sid` cookie is present** (`src/routes/main.tsx`), which is what makes `Vary: Cookie` load-bearing on this route (see Caching policy above)
  The “Reading may be solitary” community section includes readers, books tracked, and books finished in the last 7 days via `src/utils/communityStats.ts`. A single-column checkmark list replaces the feature bullets beneath the community copy, beside Recent Activity on desktop. Each row has a left-aligned label and a right-aligned, normal-sized count; the weekly count uses the same row treatment. The stats have no separate heading or visible cache-refresh note. The weekly count uses finished status and `finishedAt` within the rolling seven-day window at refresh time (excluding future dates), not import/re-sync timestamps; it shares the same aggregate scan. Readers reuse `getActiveUserCount`; books tracked counts shelved `user_book` entries (not distinct catalogue titles). One global KV snapshot uses a **7-day TTL / 24-hour stale-while-revalidate**, independently of the landing activity cache. A cold miss waits for the scan; SWR still runs synchronous SQLite on the serving process. If the aggregate fails on a cold miss, the stats are omitted.
...
| `activeUsers.ts`        | `getActiveUserCount` — distinct users with at least one book on their profile (a shelfed `user_book` row, `status IS NOT NULL`), SWR-cached inside the helper (24h TTL / 1h revalidate). Consumed by the logged-out landing page's community statistics via `communityStats.ts`; signed-in Quick Stats show only personal reading totals                                                                                      |
