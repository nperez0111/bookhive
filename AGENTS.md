...
| `activeUsers.ts`        | `getActiveUserCount` — distinct users with at least one book on their profile (a shelfed `user_book` row, `status IS NOT NULL`), SWR-cached inside the helper (24h TTL / 1h revalidate). Consumed by the home page's Quick Stats                                                                                                                                                                                              |
